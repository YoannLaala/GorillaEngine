...
#include <Renderer/RenderCommandList.hpp>
...
**	Defines
******************************************************************************/
#define D3D11_ALIGNEMENT 16
#define CHECK_CONTEXT(_context_, _pointer_) if(!_pointer_) { _context_->RemoveFlag(RenderContext::EFlag::Valid); return; }
...
namespace Gorilla { namespace Renderer
{
	...
	bool Renderer::Initialize()
	{
		...
		if(FAILED(hResult))		
		{
			LOG_INTERNAL_ERROR("[Renderer] Device Creation failed (HRESULT = 0x%0X)", hResult);
			return false;
		}
		...
		for(uint32 uiScene = 0; uiScene < uiSceneCount; ++uiScene)
		{
			...
			for(uint32 uiCamera = 0; uiCamera < uiCameraCount; ++uiCamera)
			{
				...
				for(uint32 uiRenderable = 0; uiRenderable < uiRenderableCount; ++uiRenderable)
				{
					...
					pContext->Prepare(uiFrameIndex, pRenderable);
				}
	...
	void* Renderer::PushMarker(RenderContext* _pContext, const char* _szName)
	{
		...
		if(FAILED(hResult)) 
		{
			LOG_INTERNAL_ERROR("[Renderer] ID3D11DeviceContext::QueryInterface ID3DUserDefinedAnnotation failed (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
	...
	Viewport* Renderer::CreateViewport(SwapChain* _pSwapChain, uint8 _eFilter /*= 0*/)
	{
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] CreateDeferredContext failed (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
	...
	SwapChain* Renderer::CreateSwapChain(Window* _pWindow, uint32 _uiWidth, uint32 _uiHeight, EFormat::Type _eFormat)
	{
		...
		//IDXGIFactory* pD3D11InterfaceFactory = nullptr;
		//hResult = CreateDXGIFactory(__uuidof(IDXGIFactory) ,(void**)&pD3D11InterfaceFactory);
		//if(FAILED(hResult))
		//{
		//	LOG_INTERNAL_ERROR("[Renderer] Failed to create the DXGIFactory (HRESULT = 0x%0X)", hResult);
		//	return nullptr;
		//}
		...
		//do
		//{
		//	IDXGIAdapter* pD3D11InterfaceAdapter = nullptr;
		//	hResult = pD3D11InterfaceFactory->EnumAdapters(vAdapter.GetSize(), &pD3D11InterfaceAdapter);
		//	if(pD3D11InterfaceAdapter) vAdapter.Add(pD3D11InterfaceAdapter);
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] Failed to query the device interface (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] Failed to query the adapter interface (HRESULT = 0x%0X)", hResult);
			return false;
		}
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] Failed to query the factory interface (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] Failed to create the SwapChain (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] Failed to fetch the back buffer on the SwapChain (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
	...
	Buffer* Renderer::CreateBuffer(uint32 _uiElementSize, uint32 _uiElementCount, uint32 _eBindFlag, EUsage::Type _eUsage, uint32 _uiCpuFag, uint32 _uiMiscFlag, const void* _pData /*= NULL*/)
	{
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] Failed to Create Buffer (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
	...
	RenderState* Renderer::CreateRenderState(RenderState::Key _kKey)
	{
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] Failed to Create RasterizerState (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] Failed to Create DepthStencilState (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
		...
		kD3D11RenderTargetBlendDesc.SrcBlendAlpha = D3D11_BLEND_ONE;
		kD3D11RenderTargetBlendDesc.BlendOpAlpha = D3D11_BLEND_OP_ADD;
		kD3D11RenderTargetBlendDesc.DestBlendAlpha = D3D11_BLEND_ZERO;
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] Failed to Create BlendState (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
	...
	SamplerState* Renderer::CreateSamplerState(ESampler::Type _eSampler, EAddressingMode::Type _eU, EAddressingMode::Type _eV, EAddressingMode::Type _eW, EDepthComparaison::Type _eComparaison /*= EDepthComparaison::Never*/)
	{
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] Failed to Create SamplerState (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
	...
	Shader* Renderer::CreateVertexShader(uint8* _pBuffer, uint32 _uiSize, Shader::Parameter* _pParameterArray, uint32 _uiParameterCount)
	{
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] CreateVertexShader failed (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] CreateInputLayout failed (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
	...
	Shader* Renderer::CreateGeometryShader(uint8* _pBuffer, uint32 _uiSize)
	{
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] CreateGeometryShader failed (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
	...
	Shader* Renderer::CreatePixelShader(uint8* _pBuffer, uint32 _uiSize)
	{
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] CreatePixelShader failed (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
	...
	Shader* Renderer::CreateComputeShader(uint8* _pBuffer, uint32 _uiSize)
	{
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] CreateComputeShader failed (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
	...
	void Renderer::CopyRegion(RenderContext* _pContext, Texture2D* _pTexture, uint32 /*_uiLeft*/, uint32 /*_uiTop*/, uint32 _uiWidth, uint32 /*_uiHeight*/, const void* _pBuffer)
	{
		...
		/*if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] Failed to Copy region (HRESULT = 0x%0X)", hResult);
			return;
		}*/
	...
	uint8* Renderer::Map(RenderContext* _pContext, Buffer* _pBuffer, EMap::Type _eMap /*= EMap::WriteDiscard*/)
	{
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] Map failed (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
	...
	void* Renderer::CreateShaderResourceView(Texture2D* _pTexture, uint32 _uiMipCount, uint32 /*_uiCount*/, EFormat::Type _eFormat, uint32 _eFlag)
	{
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] CreateShaderResourceView failed (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
	...
	void* Renderer::CreateDepthStenciltView(Texture2D* _pTexture)
	{
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] CreateDepthStencilView failed (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
	...
	void* Renderer::CreateUnorderedAccessView(Buffer* _pBuffer, EFormat::Type _eFormat)
	{
		...
		if(FAILED(hResult))
		{
			LOG_INTERNAL_ERROR("[Renderer] CreateUnorderedAccessView failed (HRESULT = 0x%0X)", hResult);
			return NULL;
		}
	...
	//!	@brief		SetBlendState 
	//!	@date		2015-11-21
	HashMap<uint16, ID3D11BlendState*> mBlendState;
	void Renderer::SetBlendState(RenderContext* _pContext, EBlend::Type _eSource, EBlendOperation::Type _eOperation, EBlend::Type _eDestination)
	{
		ID3D11DeviceContext* pD3D11DeferredContext = static_cast<ID3D11DeviceContext*>(_pContext->GetHandle());
		
		// Find proper BlendState
		BlendState kBlendState(_eSource, _eOperation, _eDestination);
		ID3D11BlendState* pD3D11BlendState = mBlendState[kBlendState];
		if(!pD3D11BlendState)
		{
			// Create BlendState Description
			D3D11_BLEND_DESC kD3D11BlendDesc;
			ZeroMemory(&kD3D11BlendDesc, sizeof (D3D11_BLEND_DESC));
			kD3D11BlendDesc.AlphaToCoverageEnable = false;
			kD3D11BlendDesc.IndependentBlendEnable = false;
			D3D11_RENDER_TARGET_BLEND_DESC& kD3D11RenderTargetBlendDesc = kD3D11BlendDesc.RenderTarget[0];
			kD3D11RenderTargetBlendDesc.BlendEnable = true;
			kD3D11RenderTargetBlendDesc.SrcBlend = (D3D11_BLEND)_eSource;
			kD3D11RenderTargetBlendDesc.BlendOp = (D3D11_BLEND_OP)_eOperation;
			kD3D11RenderTargetBlendDesc.DestBlend = (D3D11_BLEND)_eDestination;
			kD3D11RenderTargetBlendDesc.SrcBlendAlpha = D3D11_BLEND_ONE;
			kD3D11RenderTargetBlendDesc.BlendOpAlpha = D3D11_BLEND_OP_ADD;
			kD3D11RenderTargetBlendDesc.DestBlendAlpha = D3D11_BLEND_ZERO;
			kD3D11RenderTargetBlendDesc.RenderTargetWriteMask = D3D11_COLOR_WRITE_ENABLE_ALL;
			...
			// Create BlendState
			ID3D11Device* pD3D11Device = static_cast<ID3D11Device*>(m_pDevice);
			HRESULT hResult = pD3D11Device->CreateBlendState(&kD3D11BlendDesc, &pD3D11BlendState);
			if(FAILED(hResult))
			{
				LOG_INTERNAL_ERROR("[Renderer] Failed to Create BlendState (HRESULT = 0x%0X)", hResult);
			}
			...
			{
				mBlendState.Add(kBlendState, pD3D11BlendState);
			}
		}
		...
		if(pD3D11BlendState)
		{
			static float32 aBlendFactor[] = {1.0f, 1.0f, 1.0f, 1.0f};
