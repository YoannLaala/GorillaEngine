	...
	bool DDSCooker::Serialize(FileReader& _kInputStream, FileWriter& _kOutputStream)
	{
		...
		switch(eDimension)
		{
			...
			case Renderer::EDimension::Texture3D:
			{
				...
				{
					printf("[Texture] Texture3D Dimension not supported [W:%d][H:%d][D:%d][A:%d] (MAX [W:%d][H:%d][D:%d][A:%d])", kHeader.GetWidth(), kHeader.GetHeight(), kHeader.GetDepth(), uiArrayCount, D3D11_REQ_TEXTURE3D_U_V_OR_W_DIMENSION, D3D11_REQ_TEXTURE3D_U_V_OR_W_DIMENSION, D3D11_REQ_TEXTURE3D_U_V_OR_W_DIMENSION, 1);
					return NULL;
				}
				break;
			}
		...
		for(uint32 uiArray = 0; uiArray < uiArrayCount; ++uiArray)
		{
			for(uint32 uiMipMap = 0; uiMipMap < uiMipMapCount; ++uiMipMap)
			{
				...
				Renderer::TextureHelper::GetSurfaceInfo(uiWidth, uiHeight, (Renderer::EFormat::Type)eFormat, uiRowByte, uiSurfaceByte);
